...
function handleCardClick(card){let s=null;try{s=state}catch{}const g=s?.game,me=g?.players?.find(p=>p.id===s.viewerId);if(!g||!me||me.finished)return;if(g.phase==='tax'&&g.currentPlayerId===s.viewerId){toggleOne(card.dataset.id);return}if(g.phase!=='play'||g.currentPlayerId!==s.viewerId)return;const ids=autoGroupIds(card);if(!ids.length){const need=g.pile?.count||1;toast(`${need}장 묶음을 만들 카드가 부족합니다.`);return}setSelected(ids)}
