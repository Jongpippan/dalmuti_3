...
function handleCardClick(card){let s=null;try{s=state}catch{}const g=s?.game,me=g?.players?.find(p=>p.id===s.viewerId);if(!g||!me||me.finished)return;if(g.phase==='tax'&&g.tax?.pending){toggleOne(card.dataset.id);return}if(g.phase!=='play')return;const ids=autoGroupIds(card);if(!ids.length){const need=g.pile?.count||1;toast(`${need}장 묶음을 만들 카드가 부족합니다.`);return}const current=selectedIds(),same=current.size===ids.length&&ids.every(id=>current.has(id));setSelected(same?[]:ids)}
