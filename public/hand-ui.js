...
function quickSelect(card){
 const play=$('#actions [data-a="play"]');
 if(!play){toast('내 차례에 카드를 클릭하면 필요한 장수만큼 자동 선택됩니다.');return}
 const ids=autoGroupIds(card);
 if(!ids.length){let need=1;try{need=state?.game?.pile?.count||1}catch{}toast(`${need}장 묶음을 만들 카드가 부족합니다.`);return}
 setSelected(ids)
}
function bindStatic(){
...
document.addEventListener('click',e=>{
 ...
 const play=$('#actions [data-a="play"]');if(!play)return;
 e.preventDefault();e.stopImmediatePropagation();quickSelect(card)
},{capture:true});
