...
function autoGroupIds(card){const all=cards(),rank=rankOf(card);let pile=null;try{pile=state?.game?.pile||null}catch{}if(!pile){if(rank===13)return all.filter(c=>rankOf(c)===13).map(c=>c.dataset.id);return [...all.filter(c=>rankOf(c)===rank),...all.filter(c=>rankOf(c)===13)].map(c=>c.dataset.id)}const need=pile.count||1;if(need===1)return[card.dataset.id];if(rank===13)return[];const same=all.filter(c=>rankOf(c)===rank),jokers=all.filter(c=>rankOf(c)===13),ordered=[card,...same.filter(c=>c!==card)],ids=ordered.slice(0,need).map(c=>c.dataset.id);if(ids.length<need)ids.push(...jokers.slice(0,need-ids.length).map(c=>c.dataset.id));return ids.length===need?ids:[]}
function handleCardClick(card){let s=null;try{s=state}catch{}const g=s?.game,me=g?.players?.find(p=>p.id===s.viewerId);if(!g||!me||me.finished)return;if(g.phase==='tax'&&g.tax?.pending){toggleOne(card.dataset.id);return}if(g.phase!=='play'||g.currentPlayerId!==s.viewerId)return;const ids=autoGroupIds(card);if(!ids.length){const need=g.pile?.count||1;toast(`${need}장 묶음을 만들 카드가 부족합니다.`);return}const current=selectedIds(),same=current.size===ids.length&&ids.every(id=>current.has(id));setSelected(same?[]:ids)}
